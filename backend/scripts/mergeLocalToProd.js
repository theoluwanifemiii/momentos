...
async function main() {
    ...
    const local = new client_1.PrismaClient({ datasources: { db: { url: localUrl } } });
    const prod = new client_1.PrismaClient({ datasources: { db: { url: prodUrl } } });
    try {
        const orgs = await local.organization.findMany();
        if (orgs.length > 0) {
            await prod.organization.createMany({ data: orgs, skipDuplicates: true });
        }
        const prodOrgIds = new Set((await prod.organization.findMany({ select: { id: true } })).map((org) => org.id));
        ...
        const prodTemplateMap = new Map(prodTemplates.map((template) => [templateKey(template), template]));
        ...
        const users = (await local.user.findMany()).filter((user) => prodOrgIds.has(user.organizationId));
        if (users.length > 0) {
            await prod.user.createMany({ data: users, skipDuplicates: true });
        }
        const prodUserIds = new Set((await prod.user.findMany({ select: { id: true } })).map((user) => user.id));
        const onboarding = (await local.onboardingProgress.findMany()).filter((row) => prodOrgIds.has(row.organizationId));
        ...
        const people = (await local.person.findMany()).filter((person) => prodOrgIds.has(person.organizationId));
        if (people.length > 0) {
            await prod.person.createMany({ data: people, skipDuplicates: true });
        }
        const prodPersonIds = new Set((await prod.person.findMany({ select: { id: true } })).map((person) => person.id));
        const prodTemplateIds = new Set((await prod.template.findMany({ select: { id: true } })).map((template) => template.id));
        const deliveryLogs = (await local.deliveryLog.findMany()).filter((log) => prodOrgIds.has(log.organizationId) &&
        ...
        const schedulerRuns = (await local.schedulerRun.findMany()).filter((run) => prodOrgIds.has(run.organizationId));
        ...
        const orgTemplates = (await local.organizationTemplate.findMany()).filter((row) => prodOrgIds.has(row.organizationId));
        ...
        const prodAdminIds = new Set((await prod.adminUser.findMany({ select: { id: true } })).map((admin) => admin.id));
        const adminInvites = (await local.adminInvite.findMany()).filter((invite) => prodAdminIds.has(invite.createdBy));
        ...
        const adminSessions = (await local.adminSession.findMany()).filter((session) => prodAdminIds.has(session.adminId));
        ...
        const adminAuditLogs = (await local.adminAuditLog.findMany()).filter((log) => prodAdminIds.has(log.adminId));
        ...
        const otps = (await local.otp.findMany()).filter((otp) => !otp.userId || prodUserIds.has(otp.userId));
        if (otps.length > 0) {
